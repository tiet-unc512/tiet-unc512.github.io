:PROPERTIES:
:ID:       5169eb80-0ad1-4dba-b125-062ec167e65b
:END:
#+TITLE: Introduction to AR
#+AUTHOR: Raghav B. Venkataramaiyer
#+DATE: Apr '26
#+options: toc:2 num:3 h:4

#+latex_header: \usepackage{parskip}

#+latex_header_extra: \graphicspath{{./images/}{./assets/}}
...
#+md: [:material-file-pdf-box: Download this page as PDF](./scribe.pdf)
...
#+begin_src emacs-lisp
  ...
  (defun bvr/mkdocs/publish-hook ()
    ...
    (with-current-buffer (find-file-noselect (expand-file-name "scribe.md" default-directory))
      (save-excursion
        (goto-char (point-min))
        ;; (insert "---\ntoc_depth: 2\n---\n\n")
        (let* ((i 1)
...
:PROPERTIES:
:CUSTOM_ID: sec:pin-hole-camera-model
:END:
...
@@latex:\S~@@ref:sec:measurement-update) to “collapse”
...
@@latex:\S~@@ref:sec:pin-hole-camera-model.  We had
...
** Measurement Update
:PROPERTIES:
:CUSTOM_ID: sec:measurement-update
:END:
\begin{align*}
  \boldsymbol{\mu}_t
...
:PROPERTIES:
:CUSTOM_ID: sec:nonlinear-least-squares
:END:
...
@@latex:\S~@@ref:sec:nonlinear-least-squares) boils
...
#+name: fig:slam-graph
#+caption: Bipartite graph with circles representing variable nodes and squares, the factor nodes.
[[file:assets/slam_graph_3.pdf]]
...
#+latex: \centering
#+latex: \color{white}
#+latex: \def\svgwidth{0.7\textwidth}
#+latex: \input{assets/bipartite-graph.pdf_tex}
#+latex: \caption{Example Bipartite Graph. Courtesy: Wikipedia}
#+latex: \label{fig:bipartite-graph}
#+latex: \end{figure}
...
#+attr_latex: :environment tabularx :align lXX :width \textwidth
|------------------+-------------------------------------+-------------------------------------------------------|
| *Feature*        | *Standard QP*                       | *LM Algorithm*                                        |
|------------------+-------------------------------------+-------------------------------------------------------|
| Curvature        | Exact Hessian ($H$)                 | Approximate Hessian, /i.e./ ($J^{\top}J+\lambda I$)   |
...
#+attr_latex: :environment tabularx :align llX :width \linewidth
| *Tech* | *Primary Goal*      | *Math/Tooling*                                |
|--------+---------------------+-----------------------------------------------|
...
* COMMENT Probabilistic Data Association
